...
export default function RightRail({ children }: { children: React.ReactNode }) {
  ...
  return (
    <RightRailContext.Provider value={{ mount, setFilled, setTitle, open, setOpen }}>
      ...
      <div className="lg:mr-[320px]">{children}</div>
      ...
      <aside
        className={`fixed top-0 right-0 h-screen w-[320px] z-40 flex flex-col transition-transform duration-250 ease-out lg:translate-x-0 ${open ? 'translate-x-0 shadow-2xl' : 'translate-x-full lg:shadow-none'}`}
