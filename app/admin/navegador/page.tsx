...
// Render del icono (imagen subida, integrado por id, o placeholder)
function IconPreview({ tab, size = 24 }: { tab: { id: string; icon: string }; size?: number }) {
  if (tab.icon) {
    // eslint-disable-next-line @next/next/no-img-element
    return <img src={tab.icon} alt="" style={{ width: size, height: size, objectFit: 'contain' }} />
  }
...
export default function AdminNavegadorPage() {
  ...
  const setColor = (k: 'bg' | 'accent' | 'inactive' | 'border', v: string) => setCfg(c => ({ ...c, [k]: v }))
    ...
    <div className="min-h-screen md:ml-[240px] md:pt-16" style={{ backgroundColor: S.bg }}>
      ...
      <div className="max-w-[760px] mx-auto p-4 space-y-4">
                ...
                {previewItems.map((t, i) => {
                  ...
                  return (
                    <div key={t.id} className="flex-1 py-2.5 flex flex-col items-center gap-0.5 relative"
                      style={{ color: activeTab ? cfg.accent : cfg.inactive }}>
                      {activeTab && (
        ...
        <div className="rounded-2xl p-5 space-y-3" style={{ backgroundColor: S.card, border: `1px solid ${S.border}` }}>
          ...
          {cfg.tabs.length === 0 && (
            <p className="text-xs text-center py-2" style={{ color: S.sub }}>Sin botones. Agrega al menos uno.</p>
          )}
          <p className="text-xs" style={{ color: S.sub }}>
            Sugerencia: sube iconos cuadrados (PNG/SVG) para que se vean nítidos. Si no subes icono, los botones “menu”, “review” y “card” usan su icono integrado.
          </p>
        </div>
