...
type View = 'platillos' | 'personalizacion' | 'navegador'
...
export default function AdminMenuPage() {
  const [view, setView] = useState<View>('platillos')
  ...
  return (
    <div className="min-h-screen md:ml-[240px] md:pt-16" style={{ backgroundColor: S.bg }}>
      ...
      <div className="max-w-[1200px] mx-auto p-4 space-y-4">
        ...
        <div className="flex items-center justify-between pt-1">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: S.sub }}>
              {view === 'platillos' && 'Agregar, editar y controlar disponibilidad'}
              {view === 'personalizacion' && 'Logo, colores y carrusel del menú público'}
              {view === 'navegador' && 'Editor de la barra de navegación del cliente'}
            </p>
          </div>
          {view === 'platillos' && (
        ...
        {/* Tabs */}
        <div className="flex gap-1 p-1 rounded-2xl" style={{ backgroundColor: S.card, border: `1px solid ${S.border}` }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setView(t.id)}
              className="flex-1 py-2 rounded-xl text-sm font-bold transition-all"
              style={view === t.id
                ? { backgroundColor: S.accent, color: '#000' }
                : { backgroundColor: 'transparent', color: S.sub }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {/* ── TAB: Navegador ── */}
        {view === 'navegador' && <NavegadorEditor />}
